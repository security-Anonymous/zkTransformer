...
{
    timer T;
    int logn = C.circuit[0].bit_length;
    u64 n_sqrt = 1ULL << (logn - (logn >> 1));
    vector<G> gens(n_sqrt);
    int c=0;
    G1 base=gen_gi(gens.data(),n_sqrt);
    gens.push_back(base);
    T.start();
    comm=p->commitInput(gens,commit_thread);  //commit input
    T.stop();
    p->proof_size+= 1<<(comm.l/2);
    cout<<"Model weight commit time: "<<T.elapse_sec()<<endl;
